...
fn codegen_and_compile_module(
    tcx: TyCtxt<'_>,
    codegen_mode: CodegenMode,
) -> UnwindModule<JITModule> {
    ...
    tcx.sess.time("codegen mono items", || {
        crate::base::predefine_mono_items(tcx, &mut jit_module, &mono_items);
        for (mono_item, _) in mono_items {
            match mono_item {
                ...
                MonoItem::GlobalAsm(item_id) => {
                    ...
                    tcx.dcx().span_fatal(item.span, "Global asm is not supported in JIT");
                }
            }
        }
    });
    ...
    if !cx.global_asm.is_empty() {
        tcx.dcx().fatal("Inline asm is not supported in JIT");
    }
...
fn codegen_and_compile_fn<'tcx>(
    ...
    cranelift_codegen::timing::set_thread_profiler(Box::new(crate::base::MeasuremeProfiler(
        tcx.prof.clone(),
    )));
...
pub type MainFunction = extern "C" fn(c_int, *const *const c_char) -> c_int;

fn jit_main(jit_module: &mut UnwindModule<JITModule>) -> MainFunction {
    ...
    let main: MainFunction = unsafe { ::std::mem::transmute(finalized_start) };

    main
}

pub fn start_jitted_main(jitted_main: MainFunction, args: Vec<CString>) -> c_int {
    let argv = args.iter().map(|arg| arg.as_ptr()).collect::<Vec<_>>();

    // Push a null pointer as a terminating argument. This is required by POSIX and
    // useful as some dynamic linkers use it as a marker to jump over.
    let argv = argv.into_iter().chain(std::iter::once(std::ptr::null())).collect::<Vec<_>>();

    jitted_main(argv.len() as c_int, argv.as_ptr())
}
...
pub fn run_jit(tcx: TyCtxt<'_>, codegen_mode: CodegenMode, jit_args: Vec<String>) -> ! {
    ...
    let mut jit_module = crate::jit::codegen_and_compile_module(tcx, codegen_mode);

    let jitted_main = jit_main(&mut jit_module);
    ...
    std::thread::spawn(move || {
        let ret = start_jitted_main(jitted_main, args);
