...
pub(crate) struct UnwindModule<T> {
    ...
    cache: Option<Box<dyn CacheKvStore + Send + Sync>>,
}
...
impl<T: Module> UnwindModule<T> {
    pub(crate) fn new(mut module: T, cgu_name: &str, pic_eh_frame: bool) -> Self {
        ...
        let cache = match std::env::var("CG_CLIF_FUNCTION_CACHE").as_deref() {
            Err(_) => None,
            Ok("naive") => Some(Box::new(FileCache) as Box<dyn CacheKvStore + Send + Sync>),
            Ok("module") => {
                Some(Box::new(ModuleCache::new(cgu_name)) as Box<dyn CacheKvStore + Send + Sync>)
            }
            _ => panic!(),
        };
...
struct ModuleCache {
    file: PathBuf,
    entries: FxHashMap<[u8; 32], (OwnedSlice, AtomicBool)>,
}
...
impl ModuleCache {
    fn new(name: &str) -> Self {
        ...
        while !data.is_empty() {
            ...
            entries.insert(
                key,
                (data.clone().slice(|data| &data[20..size as usize + 36]), AtomicBool::new(false)),
            );
...
impl CacheKvStore for ModuleCache {
    ...
    fn insert(&mut self, key: &[u8], val: Vec<u8>) {
        self.entries
            .insert(key.try_into().unwrap(), (slice_owned(val, |val| &val), AtomicBool::new(true)));
    }
}
