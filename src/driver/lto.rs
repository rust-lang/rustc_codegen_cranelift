...
impl OngoingCodegen {
    pub(crate) fn join(
        ...
        if !self.only_rlib {
            for (name, module) in load_lto_modules(sess, crate_info) {
                modules.push(compile_module_to_object(sess, outputs, module, name).unwrap());
            }
        }
...
fn compile_module_to_object(
    ...
    let mut module = aot::make_module(sess, name.clone());
