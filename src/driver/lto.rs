...
fn make_module(sess: &Session, backend_config: &BackendConfig) -> SerializableModule {
...
fn compile_module_to_object(
    ...
    let mut module = aot::make_module(tcx.sess, backend_config, name.clone());
        ...
        let codegen_result = cx
            ...
            .run(|| {
                if tcx.crate_types() != [rustc_session::config::CrateType::Rlib] {
                    compile_module_to_object(tcx, &backend_config, module, cgu_name.clone())
                } else {
...
pub(crate) fn run_aot(
    ...
    for (name, module) in load_lto_modules(tcx, &crate_info, &backend_config) {
        modules.push(compile_module_to_object(tcx, &backend_config, module, name).unwrap());
    }

    let allocator_module = aot::emit_allocator_module(tcx, &backend_config);
