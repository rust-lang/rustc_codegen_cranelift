...
pub(super) struct SerializableModule {
    isa: Arc<dyn TargetIsa + Send + Sync>,
...
struct SerializableModuleInner {
    ...
    functions: BTreeMap<FuncId, FunctionMaybeCompiled>,
...
impl StableHash for SerializableModule {
    fn stable_hash<Hcx: StableHashCtxt>(&self, hcx: &mut Hcx, hasher: &mut StableHasher) {
        ...
        postcard::to_stdvec(&self.inner).unwrap().stable_hash(hcx, hasher);
    }
}
...
impl SerializableModule {
    ...
    pub(crate) fn serialize(&mut self) -> &[u8] {
        self.compile_funcs();
    ...
    fn compile_funcs(&mut self) {
        let mut ctx = Context::new();
        for func in self.inner.functions.values_mut() {
            match func {
    ...
    pub(crate) fn apply_to(mut self, module: &mut dyn Module) -> String {
        self.compile_funcs();
        ...
        for (func_id, func) in self.inner.functions {
            ...
            let FunctionMaybeCompiled::Compiled(buffer, params) = func else { unreachable!() };
...
impl Module for SerializableModule {
    ...
    fn define_function_with_control_plane(
        ...
        self.inner.functions.insert(func_id, FunctionMaybeCompiled::Ir(ctx.func.clone()));
