...
impl SerializableModule {
    ...
    pub(crate) fn serialize(self, global_asm: String) -> Vec<u8> {
        postcard::to_stdvec(&(self.inner, global_asm)).unwrap()
    }
    ...
    pub(crate) fn deserialize(
        ...
        let (inner, global_asm) = postcard::from_bytes(blob).unwrap();
        (SerializableModule { isa, inner }, global_asm)
    }
